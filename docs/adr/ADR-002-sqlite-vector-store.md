...
- Relational metadata, cascade deletion, and vectors share one local database and `DocumentDbContext`.
...
The application currently stores its database at `%LOCALAPPDATA%\FPTEnglishRAG\app.db`. SQLite files and local settings are ignored by Git. Gemini keys must use user-secrets or the `GEMINI_API_KEY` environment variable and are not part of this component.
